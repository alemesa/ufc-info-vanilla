...
axios
  .get(
    'https://cors-anywhere.herokuapp.com/http://ufc-data-api.ufc.com/api/v3/iphone/fighters/title_holders'
  )
    ...
    for (var i = 0; i < length; i++) {
      ...
      fightersToDisplay.innerHTML += `<div class="championProfile">
											<div class="profilePic">
												<img src="${fighters[i].profile_image}" alt="${fighters[i]
        .first_name}">
											</div>
											<div class="profileData">
												...
												<a href="${fighters[i].link}" target="_blank">${fighters[i]
        .first_name} ${fighters[i].last_name}</a>
												<p>${fighters[i].nickname}</p>
												<p>${getWeight(fighters[i].weight_class)} lbs.</p>
												<p>
													<span class="wins">${fighters[i]
                            .wins}</span><span class="losses">-${fighters[i]
        .losses}-</span>
...
axios
  .get(
    'https://cors-anywhere.herokuapp.com/http://ufc-data-api.ufc.com/api/v3/iphone/events'
  )
      ...
      if (eventYear > year || (month <= eventMonth && year <= eventYear)) {
   	...
   	console.log('coming this month');
        ...
        if (month == eventMonth && year == eventYear && day > eventDay) {
          ...
          const defaultImage =
            'http://imagec.ufc.com/http%253A%252F%252Fmedia.ufc.tv%252Ffeatures%252F019907_WEB_EventPlaceholderRebrand_PPV.jpg?-mw500-mh500-tc1';
          if (events[i].feature_image == defaultImage) {
            events[i].feature_image = '../images/placeholder_event.jpg';
          }

          eventsToDisplay.innerHTML =
            `<div class="eventProfile">
												<div class="eventPic">
													<img src="${events[i].feature_image}" alt="${events[i]
              .base_title}" />
												</div>
												<div class="eventData">
													...
													<div class="extraInfo">
														<a href="${events[i]
                              .ticketurl}"><img class="icon" src="../images/icons/ticket.svg" alt="Ticket Icon"/>&nbsp&nbspTickets</a>
														<a href="${events[i]
                              .trailerurl}"><img class="icon" src="../images/icons/video-camera.svg" alt="Video Icon"/>&nbsp&nbspTrailer</a>
													</div>
...
axios
  .get(
    'https://cors-anywhere.herokuapp.com/http://ufc-data-api.ufc.com/api/v3/iphone/news'
  )
...
function getFighters() {
  ...
  fighterQuery.forEach(fighter =>
    fighter.addEventListener(
      'click',
      function displayFighter(e) {
        ...
        axios
          .get(
            `https://cors-anywhere.herokuapp.com/http://ufc-data-api.ufc.com/api/v3/iphone/fighters/${this
              .dataset.id}.json`
          )
          .then(function(response4) {
            ...
            searchDiv.innerHTML = `<div class="championProfile">
											...
											<div class="specificData">
												<p>${searchFighter.strengths
                          ? 'Strengths: "' + searchFighter.strengths + '"'
                          : ''}</p>
